...
::: column
```{r}
t.test(response ~ group, data = df_with_outlier)
```

Results indicate that the two groups are **not** significantly different (t~18~ = -2.1, p = 0.06).
:::
::::
...
beetle %>%
  filter(SIZE == "small") %>%
  pull(BEETLES) %>%
  wilcox.test(mu = 100) # hypothesised mean
...
- It is also more *relaxed* in that it does not assume symmetry in the distribution of the data.
